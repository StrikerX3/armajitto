...
static constexpr auto kNormalizedModeIndices = [] {
    std::array<uint32_t, 32> indices{};
    indices.fill(0);
    indices[static_cast<uint32_t>(Mode::User)] = 0;
    indices[static_cast<uint32_t>(Mode::FIQ)] = 1;
    indices[static_cast<uint32_t>(Mode::IRQ)] = 2;
    indices[static_cast<uint32_t>(Mode::Supervisor)] = 3;
    indices[static_cast<uint32_t>(Mode::Abort)] = 4;
    indices[static_cast<uint32_t>(Mode::Undefined)] = 5;
    indices[static_cast<uint32_t>(Mode::System)] = 0;
    return indices;
}();
...
inline size_t NormalizedIndex(Mode mode) {
    return kNormalizedModeIndices[static_cast<size_t>(mode)];
}
